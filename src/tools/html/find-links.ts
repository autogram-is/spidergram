...
export function findLinksInSitemap(input: string | cheerio.Root) {
  ...
  const results: FoundLink[] = [];
  $('urlset url').toArray().forEach(element => {
    results.push({
  ...
  $('sitemapset sitemap').toArray().forEach(element => {
    results.push({
      url: $(element).find('loc').text(),
      date: $(element).find('lastmod').text(),
      label: 'sitemap'
...
export function findLinksInFeed(input: string | cheerio.Root) {
  const $ = (typeof(input) === 'string') ? parseWithCheerio(input, { xmlMode: true }) : input;
  const results: FoundLink[] = [];
  $('channel item').toArray().forEach(element => {
    results.push({
  ...
  $('feed entry').toArray().forEach(element => {
    results.push({
...
export function getLinkElementAttributes(element: cheerio.Element, $: cheerio.Root) {
