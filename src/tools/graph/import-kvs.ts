...
export async function importKvs(store: string | KeyValueStore, data: unknown): Promise<KeyValueStore> {
  const kvs = (typeof store === 'string') ? await KeyValueStore.open(store) : store;
  if (isAnyJson(data)) {
