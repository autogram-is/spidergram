...
export class Dataset<Data extends Record<string, unknown> = Record<string, unknown>> extends GenericStore {
  protected static _prefix = 'ds_';
  protected static datasets = new Map<string, Dataset>();

  static async open<Data extends Record<string, unknown> = Record<string, unknown>>(name = 'default', options: CreateCollectionOptions = {}): Promise<Dataset<Data>> {
  ...
  static async pushData<Data extends Record<string, unknown> = Record<string, unknown>>(data: Data | Data[]) {
    const dataset = await this.open();
    return dataset.pushData(data);
  }

  static async getData<Data extends Record<string, unknown> = Record<string, unknown>>(options: DatasetDataOptions = {}): Promise<DatasetContent<Data>> {
  ...
  async getData(options: DatasetDataOptions = {}): Promise<DatasetContent<Data>> {
    ...
    const db = await this.db();
    const limit = opt.limit ? aql`LIMIT ${opt.offset ?? 0}, ${opt.limit}` : undefined;

    const cursor = await db.query<Data>(aql`FOR doc IN ${this.collection} ${limit} RETURN doc`, { count: true });
    let items: Data[] = [];
    if (opt.clean) {
