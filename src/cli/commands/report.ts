...
function buildFilter(input: string): AqFilter {
  ...
  if (input.indexOf('!=') >= 0) {
    ...
    filterSpec = { path: prop, negate: true };
    filterSpec['eq'] = coerceValue(value);

  } else if (input.indexOf('=') >= 0 || input.indexOf('==') >= 0) {
    // property must equal
    const [prop, value] = input.split(/=+/);
    filterSpec = { path: prop };
    filterSpec['eq'] = coerceValue(value);
    ...
    const [prop, value] = input.split('>');
    filterSpec = { path: prop };
    filterSpec['gt'] = coerceValue(value) ?? undefined;

  } else if (input.indexOf('<') >= 0) {
    // property must be less than value
    const [prop, value] = input.split('<');
    filterSpec = { path: prop };
    filterSpec['lt'] = coerceValue(value) ?? undefined;

  } else if (input.indexOf('{') >= 0) {
    // property must be contained in a list of values
    const [prop, value] = input.split('{');
    filterSpec = { path: prop };
    filterSpec['in'] = arrify(splitMultiValues(value));

  } else if (input.indexOf('}') >= 0) {
    // value must be contained in property, which is a list
    const [prop, value] = input.split('}');
    filterSpec = { path: prop };
    filterSpec['contains'] = coerceValue(value);

  } else {
    // No equality/comparison operator or explicit value. We'll treat
    // it as a 'not null'
    filterSpec = { path: input, eq: null, negate: true };
  }
...
function coerceValue(value: string) {
  ...
  else if (value.toLocaleLowerCase() === 'true') return 1;
  else if (value.toLocaleLowerCase() === 'false') return 0;
  else return value;
}