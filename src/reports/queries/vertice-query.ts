 ...
 */
export class VerticeQuery<
  V extends Vertice = Vertice,
  O extends VerticeQueryOptions = VerticeQueryOptions,
> {
  count?: number;
  db?: Database;
  ...
  get filter(): GeneratedAqlQuery {
    const keys = this.options.keys
      ? aql`FILTER item._key IN ${arrify(this.options.keys)}`
      : undefined;
    const ids = this.options.ids
      ? aql`FILTER item._id IN ${arrify(this.options.ids)}`
      : undefined;
  ...
  async run(): Promise<V[]> {
    ...
    return this.db
      .query<JsonMap>(this.query, { fullCount: true })
      .then(cursor => {
